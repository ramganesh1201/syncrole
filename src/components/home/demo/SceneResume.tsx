...
export function SceneResume({ isActive }: { isActive: boolean }) {
  ...
  return (
    ...
    >
      {/* Resume Document */}
      <div className="flex-1 max-w-sm mx-auto relative h-full glass rounded-xl border border-white/5 p-6 flex flex-col overflow-hidden bg-white/5 shadow-2xl">
        <AnimatePresence>
          {phase < 2 && (
            <motion.div
              exit={{ opacity: 0, scale: 0.9 }}
              className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md rounded-xl border-2 border-dashed border-white/20"
            >
              <motion.div
                animate={phase === 1 ? { y: [0, -10, 0] } : {}}
                transition={{ repeat: Infinity, duration: 1 }}
              >
                <UploadCloud className="w-12 h-12 text-aurora mb-3" />
              </motion.div>
              <div className="text-sm font-semibold">Drop resume here</div>
            </motion.div>
          )}
        </AnimatePresence>

        {/* Fake Resume Content */}
        <div className="space-y-4 opacity-80 blur-[1px]">
          ...
          <div className="h-4 w-1/3 bg-white/20 rounded-md mt-6 mb-2" />
          <div className="space-y-2">
      ...
      <div className="flex-1 flex flex-col gap-4">
        {phase >= 3 && (
          <motion.div
            ...
            className="glass rounded-xl p-6 border border-white/5 flex items-center justify-between"
          >
            <div>
              <div className="text-sm text-muted-foreground uppercase tracking-widest mb-1">ATS Match Score</div>
