...
import { DSAWorkspaceService, WorkspaceProblem, SampleTestCase } from "@/lib/services/dsa-workspace.service";
import { DSASubmissionService, SubmissionResult, SubmissionStatus, validateSourceCode } from "@/lib/services/dsa-submission.service";
import { useDSAPracticeSession } from "@/hooks/useDSAPracticeSession";
import { useDSACodeDraft } from "@/hooks/useDSACodeDraft";

// Lazy-load heavy components so the rest of SyncRole is not slowed down
...
function DSAWorkspacePage() {
  ...
  const [submissionCount, setSubmissionCount] = useState(0); // for attempt_number
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (prob && !prob.has_internal_engine) {
          // External problem — redirect to the external link or back to library
          if (prob.leetcode_url) {
            window.location.href = prob.leetcode_url;
          } else {
            setLoadError("This problem does not have an internal workspace yet.");
          }
        }
      } catch (err: any) {
        if (!cancelled) {
          setLoadError(err.message ?? "Failed to load problem.");
        }
      }
    }
  ...
  // Practice session tracking (active time, heartbeats)
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (result.status === "accepted") {
        ...
        // Refresh user progress after solve
        if (userId) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <header className="flex-none border-b border-white/5 bg-background/80 backdrop-blur-xl px-4 py-2">
        <div className="mx-auto max-w-screen-2xl flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            {isSolved && (
              <span className="flex-none flex items-center gap-1 text-[10px] text-neon bg-neon/10 rounded-full px-2 py-0.5 font-medium">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        <div className="hidden md:flex h-full">
          ...
          <div className="flex-1 flex flex-col h-full min-w-0">
            ...
            <div className="flex-none h-[240px] border-t border-white/5 overflow-y-auto">
      ...
      {/* Draft save indicator (tiny, non-intrusive) */}
      {draft.hasDraft && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
